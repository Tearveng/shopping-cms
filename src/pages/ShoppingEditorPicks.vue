<template>
  ...
  >
    <a-typography-text>Shopping editor picks - 350 x 350 px</a-typography-text>
    <br />
    <br />
    <a-form-item
      v-for="(editor, index) in dynamicValidateForm.editors"
      :key="editor.key"
      v-bind="formItemLayout"
    >
      <a-form-item
        style="margin-bottom: -16px"
        :name="['editors', index, 'title']"
        :rules="[
      ...
      >
        <a-flex>
          <MinusCircleOutlined
            ...
            v-if="dynamicValidateForm.editors.length > 1"
            class="dynamic-delete-button"
            @click="removeBanner(editor)"
          />
          <a-input
            name="editor.alias"
...
const dynamicValidateForm = reactive<{ editors: ShoppingEditorPick[] }>({
  editors: [],
});
