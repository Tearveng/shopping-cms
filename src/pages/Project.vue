...
import {
  ...
  getProjectExperiencesById,
  insertProjectExperiences,
  updateProjectExperiences,
  type IProjectExperiences,
...
const customUpload = ({ indexRow }: any) => {
  return async ({ file, onSuccess, onError }: any) => {
    try {
      ...
      const projectExperience = dynamicValidateForm.experiences[indexRow];
      if (projectExperience.id) {
        const getByUserId = await getProjectExperiencesById(
        ...
        const rest = {
          ...
          start_date: projectExperience.dateRange[0],
          end_date: projectExperience.dateRange[1],
          link: projectExperience.link,
          description: projectExperience.description,
          images: oldImages,
          user_id: auth.user?.id,
        } as IProjectExperiences;

        updateProjectExperiences(rest)
          .then()
          .catch((e) => errors(e))
          .finally(() => {
...
const showConfirm = (item: ProjectExperience) => {
  modal.confirm({
    title: "Delete",
    ...
    onOk() {
      if (item.id) {
        deleteProjectExperience(item.id)
          .then(() => {
...
const removeDomain = (item: ProjectExperience) => {
  if (item.id) {
    showConfirm(item);
  } else {
    const index = dynamicValidateForm.experiences.indexOf(item);
...
const fetchAllData = async () => {
  if (auth.user) {
    const projectExperiences = await getProjectExperiences(auth.user.id);
    for (const i of projectExperiences) {
      const imagesList = [];
